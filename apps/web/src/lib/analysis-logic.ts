export type LogicNode =
  ...
  | { kind: 'not'; value: LogicNode }
  | { kind: 'and' | 'or' | 'xor'; left: LogicNode; right: LogicNode };
export function parseLogic(source: string): LogicNode {
  if (!source.trim() || source.length > 1000)
    throw new Error('expressionLimit');
  const tokens =
    source.match(/[A-Za-z][A-Za-z0-9_]*|[01]|[!~&|^()+*]|\S/g) ?? [];
  ...
  function expression(min: number, depth: number): LogicNode {
    ...
    let left: LogicNode;
    if (token === '!' || token === '~' || token?.toUpperCase() === 'NOT')
      left = { kind: 'not', value: expression(4, depth + 1) };
    else if (token === '(') {
      left = expression(0, depth + 1);
      if (tokens[index++] !== ')') throw new Error('expressionInvalid');
    } else if (token === '0' || token === '1')
      left = { kind: 'constant', value: token === '1' };
    else if (
      token &&
      /^[A-Za-z][A-Za-z0-9_]*$/.test(token) &&
      !['AND', 'OR', 'XOR'].includes(token.toUpperCase())
    )
    ...
    while (index < tokens.length) {
      ...
      index++;
      left = {
        kind: level === 1 ? 'or' : level === 2 ? 'xor' : 'and',
        left,
        right: expression(level + 1, depth + 1),
      };
    }
    return left;
  }
...
export function logicVariables(node: LogicNode): string[] {
  ...
  return [
    ...new Set([...logicVariables(node.left), ...logicVariables(node.right)]),
  ].sort();
}
export function evaluateLogic(
  ...
  switch (node.kind) {
    ...
    case 'not':
      return !evaluateLogic(node.value, values);
    case 'and':
      return (
        evaluateLogic(node.left, values) && evaluateLogic(node.right, values)
      );
    case 'or':
      return (
        evaluateLogic(node.left, values) || evaluateLogic(node.right, values)
...
export function assignment(
  ...
  return Object.fromEntries(
    variables.map((variable, i) => [
      variable,
      Boolean(row & (1 << (variables.length - i - 1))),
    ]),
  );
}
...
export function simplifyLogic(variables: string[], outputs: boolean[]): string {
  ...
  let terms = ones.map((index) =>
    index.toString(2).padStart(variables.length, '0'),
  );
  ...
  while (terms.length) {
    ...
    for (let a = 0; a < terms.length; a++)
      for (let b = a + 1; b < terms.length; b++) {
        const differences = [...terms[a]].flatMap((char, i) =>
          char !== terms[b][i] ? [i] : [],
        );
        ...
        used.add(terms[a]);
        used.add(terms[b]);
        next.add(terms[a].slice(0, i) + '-' + terms[a].slice(i + 1));
      }
    for (const term of terms) if (!used.has(term)) primes.add(term);
    terms = [...next];
  }
  const uncovered = new Set(ones);
  const covers = (term: string, value: number): boolean =>
    [...term].every(
      (char, i) =>
        char === '-' ||
        Number(char) === ((value >> (variables.length - i - 1)) & 1),
    );
  ...
  while (uncovered.size) {
    ...
    const best = [...primes].sort(
      (a, b) =>
        [...uncovered].filter((n) => covers(b, n)).length -
          [...uncovered].filter((n) => covers(a, n)).length ||
        a.replaceAll('-', '').length - b.replaceAll('-', '').length,
    )[0];
    selected.push(best);
    primes.delete(best);
    for (const value of uncovered)
      if (covers(best, value)) uncovered.delete(value);
  }
  return selected
    .map((term) =>
      [...term]
        .flatMap((char, i) =>
          char === '-'
            ? []
            : [char === '0' ? `!${variables[i]}` : variables[i]],
        )
        .join(' & '),
    )
    .map((term) => `(${term})`)
...
export function analyzeLogic(source: string, comparison = '') {
  const node = parseLogic(source);
  const other = comparison.trim() ? parseLogic(comparison) : null;
  const variables = [
    ...new Set([
      ...logicVariables(node),
      ...(other ? logicVariables(other) : []),
    ]),
  ].sort();
  ...
  const rows = Array.from({ length: 2 ** variables.length }, (_, index) => {
    ...
    return {
      values,
      output: evaluateLogic(node, values),
      comparison: other ? evaluateLogic(other, values) : null,
    };
  });
  const simplified = simplifyLogic(
    variables,
  ...
  return {
    ...
    rows,
    simplified:
      simplified.length < source.trim().length ? simplified : source.trim(),
    equivalent: other
      ? rows.every((row) => row.output === row.comparison)
      : null,
  };
}
export function karnaughRows(variableCount: number): {
  rows: number[];
  columns: number[];
  columnBits: number;
  rowBits: number;
  ...
  const gray = (bits: number): number[] =>
    Array.from({ length: 2 ** bits }, (_, i) => i ^ (i >> 1));
  return {
    rows: gray(rowBits),
    columns: gray(columnBits),
    columnBits,
    rowBits,
  };
}
