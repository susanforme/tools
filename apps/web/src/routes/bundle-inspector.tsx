...
import { SourceMapStackPanel } from '@/components/protocol-tool-panels';
...
function BundleInspectorPage() {
  ...
  const [tab, setTab] = useQueryParam<'size' | 'stack'>(
    'tab',
    StringParam,
    'size',
  );
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 px-4 py-6">
      ...
      <Tabs
        value={tab}
        onValueChange={(value) => setTab(value as 'size' | 'stack')}
      >
        <TabsList>
          <TabsTrigger value="size">
            {t('protocol.tabs.bundleSize')}
          </TabsTrigger>
          <TabsTrigger value="stack">
            {t('protocol.tabs.sourceMap')}
          </TabsTrigger>
        </TabsList>
      </Tabs>
      {tab === 'stack' ? (
