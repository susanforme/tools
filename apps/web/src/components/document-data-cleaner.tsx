...
import { cleanTable, parseCleanupCsv } from '@/lib/document-workspace-core';
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        {t('documentWorkspaces.cleanLimit')}
      </p>
      <PracticalText
        multiline
        label="CSV"
      ...
      >
        {t('documentWorkspaces.load')}
      </Button>
      {rows && (
        <>
          <div className="grid gap-3 md:grid-cols-3">
            <ChoiceField
              label={t('documentWorkspaces.operation')}
              value={op}
              options={[
                'dedup',
                'split',
            ...
            />
            <ChoiceField
              label={t('documentWorkspaces.column')}
              value={String(col)}
            ...
            {op === 'merge' && (
              <ChoiceField
                label={t('documentWorkspaces.otherColumn')}
                value={String(other)}
                options={rows[0].map((label, index) => ({
                  value: String(index),
                  label: label || String(index + 1),
          ...
          {['split', 'merge', 'fill'].includes(op) && (
            <PracticalText
              label={t(
                op === 'fill'
                  ? 'documentWorkspaces.fillValue'
                  : 'documentWorkspaces.separator',
              )}
              value={value}
              onChange={setValue}
              maxLength={100}
          ...
          <p className="text-sm text-muted-foreground">
            {t(`documentWorkspaces.${op}Hint`, { defaultValue: '' })}
          </p>
          <div className="flex flex-wrap gap-2">
            <Button disabled={history.length >= 30} onClick={apply}>
              {t('documentWorkspaces.apply')}
            </Button>
            <Button
              variant="outline"
            ...
            >
              {t('documentWorkspaces.exportCsv')}
            </Button>
          </div>
          <p className="text-sm">
            {t('documentWorkspaces.rowCount', { count: rows.length - 1 })}
          </p>
          ...
          <ol className="list-inside list-decimal text-sm">
            {history.map((entry, i) => (
              <li key={i}>
                {t(`documentWorkspaces.${entry.action}`)} · {entry.rows.length - 1}
              </li>
            ))}
          </ol>
        </>
