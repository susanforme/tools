...
import type { GcodeResult } from '@/lib/audio-gcode-analysis';
...
const createWorker = () =>
  new Worker(
    new URL('../lib/audio-gcode-analysis.worker.ts', import.meta.url),
    { type: 'module' },
  );
export default function GcodeInspector() {
  ...
  return (
    <div className="space-y-4">
      <Label htmlFor="gcode-file">{t('mediaEditingTools.gcodeFile')}</Label>
      <Input
        id="gcode-file"
        type="file"
      ...
      <p className="text-sm text-muted-foreground">
        {t('mediaEditingTools.gcodeLimit')}
      </p>
      <Button
        variant="outline"
        onClick={() => {
      ...
      {result && (
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <ChoiceField
              label={t('mediaEditingTools.paths')}
              value={query.path ?? 'all'}
              options={['all', 'extrusion', 'travel'].map((value) => ({
                value,
                label: t(`mediaEditingTools.${value}`),
              }))}
              onChange={(path) => setQuery({ path })}
            />
            <ChoiceField
              label={t('mediaEditingTools.color')}
            ...
            />
            <NumberField
              label={t('mediaEditingTools.diameter')}
              value={diameter}
              min={0.1}
              max={10}
          ...
          <Label htmlFor="gcode-layer">
            {t('mediaEditingTools.layer')} {layer + 1}/
          ...
          <canvas
            ...
            aria-label={t('mediaEditingTools.titles.gcode')}
          />
          <p className="text-sm">
            {t(
              query.color === 'speed'
                ? 'mediaEditingTools.speedLegend'
                : 'mediaEditingTools.pathLegend',
            )}
          </p>
          <div className="grid gap-3 sm:grid-cols-3">
            <p>
              {t('mediaEditingTools.filament')}: {(result.filament / 1000).toFixed(3)}{' '}
              m
            </p>
            <p>
              {t('mediaEditingTools.volume')}:{' '}
            ...
            <p>
              {t('mediaEditingTools.estimatedTime')}:{' '}
              {(result.seconds / 60).toFixed(2)} min
            </p>
          </div>
          <p className="text-sm text-muted-foreground">
            {t('mediaEditingTools.timeNote')}
          </p>
        </>
      )}
    </div>
