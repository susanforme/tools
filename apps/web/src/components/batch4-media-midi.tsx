...
import {
  ...
  transformMidi,
  type MidiProject,
} from '@/lib/batch4-media-midi';
import type { MidiRequest, MidiResponse } from '@/lib/batch4-media-midi.worker';
...
const createWorker = () =>
  new Worker(new URL('../lib/batch4-media-midi.worker.ts', import.meta.url), {
    type: 'module',
  });
const PITCH_NAMES = [
  'C',
...
export default function MidiWorkbench() {
  ...
  return (
    <div className="space-y-4">
      <Label htmlFor="midi-file">{t('batch4Media.midiFile')}</Label>
      <Input
        id="midi-file"
        type="file"
      ...
      <fieldset disabled={disabled} className="min-w-0 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {t('batch4Media.applyTranspose')}
          </Button>
        </div>
        {current && (
          <>
            <Label htmlFor="midi-track-name">
              {t('batch4Media.trackName')}
            </Label>
            <Input
              id="midi-track-name"
              value={current.name}
            ...
            <div className="overflow-x-auto rounded border">
              <svg
                ...
                role="img"
                aria-label={t('batch4Media.pianoRoll')}
                onClick={(e) => {
                  if (disabled) return;
            ...
            <p className="text-sm">{t('batch4Media.rollHint')}</p>
            <div className="grid gap-3 md:grid-cols-4">
              <NumberField
                label={t('batch4Media.pitch')}
                value={pitch}
                min={0}
                max={127}
                step={1}
              ...
              />
              <NumberField
                label={t('batch4Media.lengthBeat')}
                value={length}
                min={0.0625}
                max={60000}
                step={grid}
            ...
            <div className="flex gap-2">
              ...
              {selected !== null && (
                <>
                  ...
                  >
                    {t('batch4Media.deleteNote')}
                  </Button>
                  <Button variant="outline" onClick={() => setSelected(null)}>
                    {t('batch4Media.deselect')}
                  </Button>
                </>
              )}
            </div>
            <details>
              <summary>
                {t('batch4Media.tempoMap')} ({project.header.tempos.length})
              </summary>
