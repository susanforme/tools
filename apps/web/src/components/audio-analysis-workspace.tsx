...
import type { AudioAnalysis } from '@/lib/audio-gcode-analysis';
...
export default function AudioAnalyzer() {
  ...
  return (
    <div className="space-y-4">
      <Label htmlFor="analysis-file">{t('mediaEditingTools.audioFile')}</Label>
      <Input
        id="analysis-file"
        type="file"
        accept="audio/*"
      ...
      <p className="text-sm text-muted-foreground">
        {t('mediaEditingTools.audioLimit')}
      </p>
      {(decoding || job.busy) && (
        <p role="status">{t('mediaEditingTools.loading')}</p>
      )}
      {(error || job.error) && (
      ...
      {result && (
        <>
          <div className="grid gap-3 sm:grid-cols-3">
            {(
              ...
              <div className="rounded-lg border p-3" key={label}>
                <p className="text-sm text-muted-foreground">
                  {t(`mediaEditingTools.${label}`)}
                </p>
                <p className="font-mono text-lg">{value}</p>
              </div>
            ))}
          </div>
          <h2 className="font-semibold">{t('mediaEditingTools.spectrum')}</h2>
          <svg
            viewBox="0 0 800 220"
            className="w-full rounded border bg-muted/20"
            role="img"
            aria-label={t('mediaEditingTools.spectrum')}
          >
            <path
              d={result.spectrum
                .map(
          ...
          <h2 className="font-semibold">{t('mediaEditingTools.spectrogram')}</h2>
          <canvas
            ref={canvas}
            className="h-64 w-full rounded border"
            aria-label={t('mediaEditingTools.spectrogram')}
          />
          <p className="text-sm">
            {t('mediaEditingTools.spectrogramAxes', {
