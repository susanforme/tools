...
import { type RedactRect, validateRect } from '@/lib/batch4-document-data';
import { rebuildPdf } from '@/lib/batch4-document-pdf';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        />
        <ChoiceField
          label={t('batch4Documents.crop')}
          value={String(q.crop ?? 0)}
          options={[
            { value: '0', label: t('batch4Documents.no') },
            { value: '1', label: t('batch4Documents.yes') },
          ]}
          onChange={(v) => setQ({ crop: +v })}
        />
        <ChoiceField
          label={t('batch4Documents.numbers')}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <PracticalText
          label={t('batch4Documents.header')}
          value={header}
          onChange={setHeader}
          maxLength={120}
        />
        <PracticalText
          label={t('batch4Documents.footer')}
          value={footer}
          onChange={setFooter}
          maxLength={120}
        />
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        >
          {t('batch4Documents.previewPage')}
        </Button>
      </div>
      <p className="text-sm text-muted-foreground">
        {t('batch4Documents.redactHint')}
      </p>
      {preview.result && (
        <svg
          ...
          role="img"
          aria-label={t('batch4Documents.redactPreview')}
          onPointerDown={(e) => {
      ...
      >
        {t(
          job.busy ? 'batch4Documents.processing' : 'batch4Documents.generate',
        )}
      </Button>
