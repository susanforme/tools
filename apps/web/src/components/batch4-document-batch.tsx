...
import { bindTemplate } from '@/lib/batch4-document-data';
...
export function BatchDocuments() {
  ...
  const template =
    custom ??
    t(`batch4Documents.templates.${kind}`, {
      defaultValue: t('batch4Documents.templates.certificate'),
    });
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        {t('batch4Documents.batchHint')}
      </p>
      <div className="grid gap-3 md:grid-cols-3">
        <ChoiceField
          label={t('batch4Documents.template')}
          value={kind}
        ...
        />
        <ChoiceField
          label={t('batch4Documents.format')}
          value={format}
          options={[
            { value: 'pdf', label: 'PDF' },
            { value: 'zip', label: 'ZIP' },
      ...
      <PracticalText
        label={t('batch4Documents.prefix')}
        value={prefix}
        onChange={setPrefix}
        maxLength={50}
      />
      ...
      />
      <PracticalText
        label={t('batch4Documents.templateBody')}
        value={template}
        onChange={setCustom}
        multiline
        maxLength={12000}
