...
export default function Storyboard() {
  ...
  async function sheets() {
    ...
    for (let offset = 0; offset < frames.length; offset += 6) {
      ...
      ctx.fillText(t('mediaEditingTools.titles.storyboard'), 12, 9);
  ...
  return (
    <div className="space-y-4">
      <Label htmlFor="storyboard-file">{t('mediaEditingTools.videoFile')}</Label>
      <Input
        id="storyboard-file"
        type="file"
        accept="video/*"
      ...
      <p className="text-sm text-muted-foreground">
        {t('mediaEditingTools.videoLimit')}
      </p>
      {source && (
        <video
          ref={video}
      ...
      <NumberField
        label={t('mediaEditingTools.frameInterval')}
        value={interval}
        min={0.1}
        max={36000}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {frames.map((frame, i) => (
          <div className="space-y-2 rounded-lg border p-3" key={frame.id}>
            ...
            <Label htmlFor={`frame-${frame.id}`}>{t('mediaEditingTools.note')}</Label>
            <Textarea
              id={`frame-${frame.id}`}
              value={frame.note}
              maxLength={200}
