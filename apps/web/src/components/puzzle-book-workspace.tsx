...
import {
  ...
  type PuzzleRequest,
  type PuzzleMode,
} from '@/lib/puzzle-book-generator';
import {
  printLines,
  sheetImages,
  studySheet,
...
const createWorker = () =>
  new Worker(new URL('../workers/puzzle-book.worker.ts', import.meta.url), {
    type: 'module',
  });
function drawGrid(
  ...
  return (
    <>
      <div className="mx-auto max-w-6xl px-4 pt-5">
        <ChoiceField
          label={t('scienceTools.puzzleMode')}
          value={mode}
          options={['wordSearch', 'maze', 'crossword', 'nonogram'].map(
            (value) => ({ value, label: t(`scienceTools.${value}`) }),
          )}
...
function PuzzleGenerator({
    ...
    try {
      ...
      for (let i = 0; i < worker.result.length; i++) {
        ...
        sheet.context.fillText(
          `${t(`scienceTools.${mode}`)} · ${t('scienceTools.puzzleNumber', { number: i + 1 })}`,
          15,
          15,
        );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4 px-4 py-6">
      <h1 className="text-2xl font-bold">{t(`scienceTools.${mode}`)}</h1>
      <div className="grid gap-3 md:grid-cols-3">
        <ChoiceField
          label={t('scienceTools.difficulty')}
          value={difficulty}
          options={['easy', 'medium', 'hard'].map((value) => ({
            value,
            label: t(`scienceTools.${value}`),
          }))}
          onChange={(difficulty) => onChange({ difficulty })}
        />
        <NumberField
          label={t('scienceTools.seed')}
          value={seed}
          min={0}
          max={4294967295}
          step={1}
      ...
      {mode === 'crossword' && (
        <>
          <PracticalText
            multiline
            label={t('scienceTools.words')}
            value={words}
            onChange={setWords}
            maxLength={10000}
          />
          <p className="text-sm text-muted-foreground">
            {t('scienceTools.crosswordHint')}
          </p>
        </>
      )}
      <div className="flex flex-wrap gap-2">
        ...
        >
          {t('scienceTools.generate')}
        </Button>
        {worker.busy && (
          <Button variant="outline" onClick={worker.cancel}>
            {t('scienceTools.cancel')}
          </Button>
        )}
      </div>
      {(worker.error || error) && (
        <p role="alert" className="text-destructive">
          {t(
            `scienceTools.${['generationFailed', 'TIMEOUT', 'CANCELLED', 'WORKER_ERROR'].includes(worker.error ?? '') ? worker.error : 'puzzleInvalid'}`,
          )}
        </p>
      )}
      {result && (
        <>
          <div className="flex flex-wrap gap-2">
            ...
            <ChoiceField
              ...
              options={worker.result!.map((_, i) => ({
                value: String(i),
                label: t('scienceTools.puzzleNumber', { number: i + 1 }),
              }))}
          ...
          <img
            src={images[0]}
            alt={t(`scienceTools.${mode}`)}
            className="mx-auto w-full max-w-xl border"
          />
          {result.unplaced.length > 0 && (
            <p>
              {t('scienceTools.unplaced', {
