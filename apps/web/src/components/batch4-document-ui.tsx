  ...
  return (
    <>
      <div className="mx-auto max-w-6xl px-4 pt-5">
        <ChoiceField
          label={t('batch4Documents.section')}
          ...
          options={[
            ...
            { value: 'extra', label: t(`batch4Documents.${name}`) },
          ]}
          onChange={setSection}
        />
      </div>
      {section === 'extra' ? (
        <div className="mx-auto max-w-6xl space-y-4 px-4 py-6">
          <h1 className="text-2xl font-bold">{t(`batch4Documents.${name}`)}</h1>
          {extra}
        </div>
      ) : (
        base
...
export function DocumentError({ error }: { error: string | null }) {
  ...
  return error ? (
    ...
    >
      {t('batch4Documents.error')}{' '}
      {error.startsWith('variable:')
        ? error.slice(9)
        : t(`batch4Documents.errors.${error}`, {
            defaultValue: t('batch4Documents.errors.invalid'),
          })}
    </p>
  ) : null;
}
