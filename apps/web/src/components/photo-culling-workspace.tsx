...
export default function PhotoWorkbench() {
  ...
  return (
    <div className="space-y-4">
      <Label htmlFor="photo-files">{t('mediaEditingTools.photoFiles')}</Label>
      <Input
        id="photo-files"
        type="file"
        multiple
      ...
      <p className="text-sm text-muted-foreground">
        {t('mediaEditingTools.photoLimit')}
      </p>
      <div className="grid gap-3 md:grid-cols-3">
        <NumberField
          label={t('mediaEditingTools.zoom')}
          value={q.zoom ?? 1}
          min={0.1}
          max={10}
          step={0.1}
        ...
        />
        <NumberField
          label={t('mediaEditingTools.panX')}
          value={q.x ?? 50}
          min={0}
          max={100}
          onChange={(x) => setQ({ x })}
        />
        <NumberField
          label={t('mediaEditingTools.panY')}
          value={q.y ?? 50}
          min={0}
          max={100}
          onChange={(y) => setQ({ y })}
        />
        <ChoiceField
          label={t('mediaEditingTools.filter')}
          value={q.filter ?? 'all'}
        ...
        />
        <NumberField
          label={t('mediaEditingTools.minStars')}
          value={q.stars ?? 0}
          min={0}
          max={5}
          step={1}
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          ...
          onClick={() => void pack()}
        >
          {t('mediaEditingTools.zipSelected')} (
        ...
        >
          {t('mediaEditingTools.clear')}
        </Button>
      </div>
      {busy && <p role="status">{t('mediaEditingTools.loading')}</p>}
      {error && (
        <p role="alert" className="text-destructive">
          {t('mediaEditingTools.failed', {
            msg: t(`mediaEditingTools.errors.${error}`, { defaultValue: error }),
          })}
        </p>
      )}
      <div className="grid gap-4 md:grid-cols-2">
        {shown.map((p) => (
          <div key={p.id} className="min-w-0 space-y-2 rounded-lg border p-3">
            ...
            <div className="flex flex-wrap gap-1">
              {Array.from({ length: 6 }, (_, rating) => (
                <Button
                  ...
                  size="sm"
                  aria-label={t('mediaEditingTools.rating', { rating })}
                  onClick={() =>
                    update(
            ...
            <ChoiceField
              label={t('mediaEditingTools.decision')}
              value={p.status}
