...
export const Route = createFileRoute('/url-encode')({ component: UrlEncodePage })
...
function UrlEncodePage() {
  ...
  const enc = useTool('Hello World! 你好世界 https://example.com/search?q=foo&page=1')
  const dec = useTool()
  const parse = useTool('https://example.com/search?q=hello+world&lang=zh-CN&page=1')
  ...
  const IOPanel = ({
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Button size="sm" variant="outline" onClick={state.clear}>{t('urlEncode.clear')}</Button>
      </div>
      {state.error && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border rounded-lg overflow-hidden">
          <div className="bg-muted/50 px-3 py-1.5 text-xs text-muted-foreground border-b">{inputLabel}</div>
          <textarea
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-4">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">{t('urlEncode.desc')}</p>
      </div>
      ...
      <Tabs defaultValue="encode">
        ...
        <TabsContent value="parse" className="mt-4">
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Button size="sm" onClick={parseUrl}>{t('urlEncode.parse')}</Button>
              <Button size="sm" variant="outline" onClick={parse.clear}>{t('urlEncode.clear')}</Button>
            </div>
            {parse.error && (
              <div className="text-sm text-destructive bg-destructive/10 px-3 py-2 rounded-md">{parse.error}</div>
            )}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="border rounded-lg overflow-hidden">
                <div className="bg-muted/50 px-3 py-1.5 text-xs text-muted-foreground border-b">{t('urlEncode.urlInput')}</div>
                <textarea
                  className="w-full h-48 p-3 font-mono text-sm bg-background resize-none focus:outline-none"
                  value={parse.input}
              ...
              <div className="border rounded-lg overflow-hidden">
                <div className="bg-muted/50 px-3 py-1.5 text-xs text-muted-foreground border-b">{t('urlEncode.parsed')}</div>
                <textarea
                  readOnly
                  className="w-full h-48 p-3 font-mono text-sm bg-muted/20 resize-none focus:outline-none"
                  value={parse.output}
