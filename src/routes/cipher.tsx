...
function useTool() {
  ...
  const clear = () => { setInput(''); setKey(''); setIv(''); setOutput(''); setError(null) }
  return { input, setInput, key, setKey, iv, setIv, output, setOutput, error, setError, loading, setLoading, clear }
}
...
function CipherPanel({
  ...
  const run = async () => {
    ...
    state.setError(null)
    try {
      const result = await cipherWithCryptoJs(op, algo, state.input, state.key, state.iv, cipherMode)
      if (!result) throw new Error('结果为空，请检查密钥或输入内容')
      state.setOutput(result)
    } catch (e) {
      state.setError(t(op === 'encrypt' ? 'cipher.encryptError' : 'cipher.decryptError', { msg: (e as Error).message }))
    } finally {
  ...
  return (
    <div className="space-y-4">
          ...
          {MODES.map((m) => (
            <button
              ...
              className={`px-2.5 py-1 rounded text-xs font-mono border transition-colors ${
                cipherMode === m ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-muted'
              }`}
            >
              {m}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">{t('cipher.key')}</label>
          <input
            type="text"
            value={state.key}
        ...
        {cipherMode !== 'ECB' && (
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">{t('cipher.iv')}</label>
            <input
              type="text"
              value={state.iv}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border rounded-lg overflow-hidden">
          ...
          <textarea
            ...
            placeholder={op === 'encrypt' ? t('cipher.plaintextPlaceholder') : t('cipher.ciphertextPlaceholder')}
            spellCheck={false}
          />
        </div>
        <div className="border rounded-lg overflow-hidden">
          <div className="flex items-center bg-muted/50 px-3 py-1.5 border-b">
            <span className="text-xs text-muted-foreground flex-1">
              {op === 'encrypt' ? t('cipher.ciphertext') : t('cipher.plaintext')}
            </span>
      ...
      {state.error && (
        <div className="text-sm text-destructive bg-destructive/10 px-3 py-2 rounded-md">{state.error}</div>
      )}

      <div className="flex items-center gap-2">
        <Button size="sm" onClick={run} disabled={state.loading || !state.input.trim() || !state.key.trim()}>
          {state.loading ? t('cipher.processing') : op === 'encrypt' ? t('cipher.encrypt') : t('cipher.decrypt')}
        </Button>
        <Button size="sm" variant="outline" onClick={state.clear}>{t('cipher.clear')}</Button>
      </div>
    </div>
  )
}
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
      ...
      <Tabs defaultValue="encrypt">
        ...
        <TabsContent value="encrypt" className="mt-4">
          <CipherPanel op="encrypt" algo={algo} cipherMode={cipherMode} setCipherMode={setCipherMode} />
        </TabsContent>
        <TabsContent value="decrypt" className="mt-4">
          <CipherPanel op="decrypt" algo={algo} cipherMode={cipherMode} setCipherMode={setCipherMode} />
        </TabsContent>
      </Tabs>
    </div>
  )
