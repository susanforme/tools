...
import { Copy, Check, AlertCircle, CheckCircle2, Clock, Plus, Trash2 } from 'lucide-react'
...
function JsonBlock({ title, data, color }: { title: string; data: Record<string, unknown>; color: string }) {
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      ...
      <pre className="p-3 font-mono text-sm bg-background overflow-x-auto leading-relaxed">{json}</pre>
    </div>
  )
}
...
function DecodeTab() {
  ...
  const clear = () => { setInput(''); setDecoded(null); setError(null) }

  const expInfo = decoded?.payload?.exp != null ? formatExpiry(decoded.payload.exp as number) : null
  const nbfInfo = decoded?.payload?.nbf != null ? new Date((decoded.payload.nbf as number) * 1000).toLocaleString() : null
  const iatInfo = decoded?.payload?.iat != null ? new Date((decoded.payload.iat as number) * 1000).toLocaleString() : null
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Button size="sm" onClick={decode} disabled={!input.trim()}>{t('jwt.decode')}</Button>
        <Button size="sm" variant="outline" onClick={clear}>{t('jwt.clear')}</Button>
      </div>
        ...
        <div className="space-y-3">
          {expInfo && (
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${
              ...
              {expInfo.expired ? <AlertCircle className="w-4 h-4 shrink-0" /> : <CheckCircle2 className="w-4 h-4 shrink-0" />}
              <span>{t('jwt.exp')}: {expInfo.text}</span>
            </div>
          )}
          {(iatInfo || nbfInfo) && (
            <div className="flex gap-4 flex-wrap text-sm text-muted-foreground">
              {iatInfo && <div className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /><span>{t('jwt.iat')}: {iatInfo}</span></div>}
              {nbfInfo && <div className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /><span>{t('jwt.nbf')}: {nbfInfo}</span></div>}
            </div>
          )}
          <JsonBlock title={t('jwt.header')} data={decoded.header} color="bg-blue-500/10 text-blue-600 dark:text-blue-400" />
          <JsonBlock title={t('jwt.payload')} data={decoded.payload} color="bg-violet-500/10 text-violet-600 dark:text-violet-400" />
          <div className="border rounded-lg overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b bg-amber-500/10 text-amber-600 dark:text-amber-400">
              <span className="text-xs font-semibold">{t('jwt.signature')}</span>
              <div className="ml-auto"><CopyButton text={decoded.signature} /></div>
            </div>
            <div className="p-3 font-mono text-sm bg-background break-all select-all">{decoded.signature}</div>
          </div>
...
const ALG_OPTIONS = ['HS256', 'HS384', 'HS512', 'RS256', 'RS384', 'RS512', 'ES256', 'none']
...
function EncodeTab() {
  ...
  const encode = () => {
    ...
    try {
      ...
      for (const f of fields) {
        ...
        else if (f.value !== '' && !isNaN(Number(f.value))) val = Number(f.value)
        else {
          try { val = JSON.parse(f.value) } catch { val = f.value }
        }
  ...
  const clear = () => { setResult(''); setError(null) }
    ...
    <div className="space-y-4">
      ...
      <div className="border rounded-lg overflow-hidden">
        ...
        <div className="p-3 grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">alg</label>
            <select
              value={alg}
        ...
        <div className="divide-y">
          {fields.map((f) => (
            <div key={f.id} className="flex items-center gap-2 px-3 py-2">
              ...
              />
              <button onClick={() => removeField(f.id)} className="text-muted-foreground hover:text-destructive transition-colors">
      ...
      <div className="flex items-center gap-2">
        <Button size="sm" onClick={encode}>{t('jwt.encode')}</Button>
        <Button size="sm" variant="outline" onClick={clear}>{t('jwt.clear')}</Button>
      </div>
      ...
      {result && (
        <div className="border rounded-lg overflow-hidden">
          ...
          <div className="p-3 font-mono text-sm bg-background break-all select-all leading-relaxed">{result}</div>
        </div>
      )}
