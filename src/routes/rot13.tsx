    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {reverseLabel && onReverse && (
          <Button size="sm" variant="outline" onClick={onReverse}>{reverseLabel}</Button>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="border rounded-lg overflow-hidden">
          <div className="flex items-center bg-muted/50 px-3 py-1.5 border-b">
            <span className="text-xs text-muted-foreground flex-1">{outputLabel}</span>
            <button
              onClick={copy}
...
function Rot13Page() {
  ...
  const applyRot13 = () => {
    rot.setError(null)
    try { rot.setOutput(rot13(rot.input)) }
    catch (e) { rot.setError(`ROT13 失败：${(e as Error).message}`) }
  }

  const encodeEntity = () => {
    entity.setError(null)
    try { entity.setOutput(encodeHtmlEntities(entity.input)) }
    catch (e) { entity.setError(`编码失败：${(e as Error).message}`) }
  }

  const decodeEntity = () => {
    entity.setError(null)
    try { entity.setOutput(decodeHtmlEntities(entity.input)) }
    catch (e) { entity.setError(`解码失败：${(e as Error).message}`) }
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      ...
      <Tabs defaultValue="rot13">
        ...
        <TabsContent value="rot13" className="mt-4">
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">{t('rot13.rot13Note')}</p>
            <PanelLayout
              state={rot}
              onAction={applyRot13}
              onClear={rot.clear}
